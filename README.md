...
- Docker/Nginx 部署下由后端提供 `/dev.html` 开发者仪表盘，可查看在线设备、历史 session 和访问 IP 汇总。
...
本地开发地址默认由 Vite 输出。`npm run dev:api` 默认监听 `http://127.0.0.1:3000/`，Vite 已把 `/api` 代理到该后端。
...
src/pages/                      首页、科目练习页、开发者仪表盘
src/components/                 共享 UI 组件
src/lib/questions.ts            题型、判分、搜索、模拟考试抽题
src/lib/progress.ts             进度保存、旧状态迁移、间隔复习、后端同步、client id
...
backend/server.js               可选 JSON session 后端、/dev.html 和开发者数据 API
...
源码根目录只保留 `index.html`。科目入口如 `linux_practice.html`、`modern_history_practice.html` 由 `npm run build` 后在 `dist/` 中自动生成。不要把科目题目或独立渲染逻辑写回 HTML。开发者仪表盘不是 React 前端页面，而是由后端直接提供的 `/dev.html`。
...
客户端会在 cookie 和 localStorage 中保存 `study_hub_client_id`。后端优先使用该 client id 命名 session 文件，因此用户 IP 变化时仍能读取同一份进度。没有 client id 时才退回按 IP 识别。

前端会在页面打开后定期触碰 `/api/session`，仪表盘的“在线”状态按最近一次 heartbeat 或进度同步时间估算。
...
`/dev.html` 由后端直接返回，登录后通过 HttpOnly cookie 访问 `/api/dev-dashboard`。仪表盘展示在线设备、历史 session、访问 IP、应用快照数量和最近路径。默认不展示完整 localStorage 内容。

开发者密码存放在后端环境变量：

```text
DEV_DASHBOARD_PASSWORD=mk113711
```

默认值是 `mk113711`，面向多人网络部署时应改成自定义密码。GitHub Pages 等纯静态部署没有后端 API，因此不提供 `/dev.html`。
...
- 开放到多人网络时必须设置 `DEV_DASHBOARD_PASSWORD`，避免开发者仪表盘被无关用户查看。
